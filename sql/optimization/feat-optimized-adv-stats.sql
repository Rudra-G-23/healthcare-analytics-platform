...
DECLARE
    ...
    target_table  TEXT := 'rate_gain_matrix_history';        -- <<< EDIT per run
    output_table  TEXT := 'rate_gain_stats_adv'; -- <<< EDIT per run
    ...
    col_mean NUMERIC;
BEGIN
    -- Change this settings acc to the your local Laptop
    ...
    ------------------------------------------------------------
    -- NUMERIC COLUMNS (2 scans: aggregates+distinct, then percentiles)
    ------------------------------------------------------------
    FOR col_record IN
    ...
    LOOP
        ...
        -- get mean first (needed as a literal inside the skewness formula)
        EXECUTE format('SELECT AVG(%I) FROM %I.%I',
            col_record.column_name, target_schema, target_table) INTO col_mean;

        -- SCAN 1: distinct count + core aggregates + skewness + min/max together
        sql_base := format('
            WITH agg AS (
                SELECT
        ...
        BEGIN
            ...
            -- SCAN 2: percentiles (needs its own sort, can't merge with aggregates above)
    ...
    ------------------------------------------------------------
    -- TEXT / CATEGORICAL COLUMNS (1 scan)
    ------------------------------------------------------------
    FOR col_record IN
        SELECT column_name, data_type
        FROM information_schema.columns
